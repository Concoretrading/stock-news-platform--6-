...
export class NemotronService {
    ...
    private modelName: string = "nvidia/nemotron-3-super-120b-a12b";
        ...
        try {
            const response = await fetch(`${this.apiUrl}/chat/completions`, {
                ...
                body: JSON.stringify({
                    ...
                    messages: [
                        { role: 'system', content: 'You are the Elite Main Brain (NVIDIA Nemotron 3 Super) of a Predator Intelligence Trading System. Your goal is to synthesize conflicting intelligence and make a final, autonomous decision based on deep reasoning and elite mindset principles (Kahneman, Dalio, etc.).' },
                        { role: 'user', content: prompt }
                    ],
                    max_tokens: 1024,
                    temperature: 0.1,
                    // Note: reasoning_budget is a specific parameter for Nemotron models supporting it
                    extra_body: {
                        reasoning_budget: request.reasoningBudget
                    }
                })
            });
