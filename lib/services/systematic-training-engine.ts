...
import { OptionsPremiumMastery, OptionsPremiumMasterySignal } from './options-premium-mastery';
import { ichimokuCloudInterpreter, IchimokuCloudAnalysis } from './ichimoku-cloud-interpreter';
...
interface MarketMove {
  ...
  premium_snapshot?: OptionsPremiumMasterySignal; // Injected by Replay Engine
  preceding_conditions: {
    ...
    technical_context?: {
      ma_50_trend: 'up' | 'down' | 'flat';
      distance_from_ma_50: number;
      distance_from_poc: number;
      ichimoku_bias: 'BULLISH' | 'BEARISH' | 'NEUTRAL';
      cloud_interaction: 'ABOVE' | 'BELOW' | 'INSIDE' | 'APPROACHING' | 'BREAKING';
    };
  };
  outcome: {
...
export class SystematicTrainingEngine {
  ...
  private premiumMastery: OptionsPremiumMastery;
  ...
  private constructor() {
    ...
    this.premiumMastery = new OptionsPremiumMastery();
  }
  ...
  private async createTrainingPlan(symbol: string, months: number = 6, overrideStart?: string, overrideEnd?: string): Promise<TrainingPlan[]> {
    console.log(`📋 Creating systematic training plan for ${symbol} (${months} months)`);

    // Use overrides if provided, otherwise default to 6 months
    const endDate = overrideEnd ? new Date(overrideEnd) : new Date('2025-02-05T21:00:00.000Z');
    const startDate = overrideStart ? new Date(overrideStart) : new Date('2024-08-05T14:30:00.000Z');
    ...
    // Create training plan for each timeframe (Micro + Macro)
    const timeframes = ['1min', '5min', '15min', '1hour', '4hour', 'day'] as const;
  ...
  async startTraining(symbol: string, months: number = 6, pillars: string[] = ['volume', 'momentum', 'support_resistance', 'premium'], overrideStart?: string, overrideEnd?: string): Promise<TrainingSession> {
    console.log(`🎯 Systematic Training start for ${symbol}`);

    // Create training plan
    const plans = await this.createTrainingPlan(symbol, months, overrideStart, overrideEnd);
  ...
  async executeTraining(session_id: string): Promise<void> {
    ...
    for (const progress of this.current_session.total_progress) {
      ...
      try {
        ...
        // Get data in smaller chunks (params: symbol, timespan, from, to)
  ...
  private async analyzeMarketStructure(bars: PolygonBar[]): Promise<MarketMove[]> {
    ...
    const lookback = 52; // Increased to 52 to fully encapsulate Senkou Span B in Ichimoku
    ...
    const getSMA = (end: number, period: number) => {
      if (end < period) return bars[end].c;
      const subset = bars.slice(end - period, end);
      return subset.reduce((sum, b) => sum + b.c, 0) / period;
    };

    // Helper to calculate Volume Point of Control (POC) over the chunk
    const calculatePOC = (start: number, end: number) => {
      const subset = bars.slice(start, end);
      if (subset.length === 0) return 0;
      // Very basic bucketing by rounding price to nearest whole number
      const profile = new Map<number, number>();
      for (const b of subset) {
        const p = Math.round((b.h + b.l + b.c) / 3);
        profile.set(p, (profile.get(p) || 0) + b.v);
      }
      let pocPrice = subset[0].c;
      let maxVol = 0;
      profile.forEach((vol, price) => {
    ...
    for (let i = lookback; i < bars.length - 10; i++) {
      ...
      if (Math.abs(price_change) > breakout_threshold || range_expansion > 1.5) {
        ...
        let premium_snapshot: OptionsPremiumMasterySignal | undefined;
        if (this.current_session?.pillars_to_train.includes('premium')) {
            const timestampIso = new Date(current.t).toISOString();
            try {
                // Determine target SPX for SPY tests
                const targetTicker = this.current_session.symbol === 'SPY' ? 'I:SPX' : this.current_session.symbol;
                premium_snapshot = await this.premiumMastery.masterPremiumAnalysis(
                    targetTicker, 
                    current.c, 
                    timestampIso
                );
            } catch (e) {
                console.error("Premium Mastery Replay Error:", e);
            }
        }

        // --- TECHNICAL ANALYSIS SNAPSHOT ---
        const poc = calculatePOC(i - lookback, i);
        const distance_from_poc = (current.c - poc) / poc;
        
        const ma50 = getSMA(i, 50);
        const ma50_prev = getSMA(i - 1, 50);
        const ma_50_trend = ma50 > ma50_prev ? 'up' : ma50 < ma50_prev ? 'down' : 'flat';
        const distance_from_ma_50 = (current.c - ma50) / ma50;

        // Ichimoku Cloud snapshot
        const closes = prev_bars.map(b => b.c);
        const highs = prev_bars.map(b => b.h);
        const lows = prev_bars.map(b => b.l);
        const vols = prev_bars.map(b => b.v);
        const ichiAnalysis = await ichimokuCloudInterpreter.analyzeCloud('SPY', closes, highs, lows, vols);
        ...
        const move: MarketMove = {
          ...
          preceding_conditions: {
            ...
            context: {
              ...
              distance_from_vwap: current.vw ? (current.c - current.vw) / current.vw : 0
            },
            technical_context: {
              ma_50_trend,
              distance_from_ma_50,
              distance_from_poc,
              ichimoku_bias: ichiAnalysis.trend_bias,
              cloud_interaction: ichiAnalysis.interaction.status
            }
          },
