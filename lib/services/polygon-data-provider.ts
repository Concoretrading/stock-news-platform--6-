...
export class PolygonDataProvider {
  ...
  async getMultiDayData(symbol: string, timespan: string, from: string, to: string): Promise<any> {
    let multiplier = 1;
    if (timespan === '15min') multiplier = 15;
    else if (timespan === '5min') multiplier = 5;
    else if (timespan === '4hour') multiplier = 4;
    
    const span = timespan.includes('min') ? 'minute' : timespan === 'day' ? 'day' : 'hour';
