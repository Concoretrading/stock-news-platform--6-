...
interface AnalyzeScreenshotResult {
  ...
  source: string;
  imageStoragePath: string;
  catalystEntryResults?: CatalystEntryResult[];
  message: string;
}
...
export async function getWatchlistTickers(userId: string): Promise<string[]> {
  try {
    ...
    const tickers = stocksSnap.docs.map((doc: any) => doc.data().ticker);
...
export async function analyzeScreenshot(userId: string, imageBuffer: Buffer, imageType: string): Promise<AnalyzeScreenshotResult> {
  try {
    ...
    const catalystEntryResults: CatalystEntryResult[] = [];
    ...
    const successCount = catalystEntryResults.filter(r => r.success).length;
