...
export const getIdToken = async () => {
  // Try to get firebaseUser from context if available
  try {
    // const { firebaseUser } = useAuth(); // This line is removed
    // if (firebaseUser && typeof firebaseUser.getIdToken === "function") {
    //   return await firebaseUser.getIdToken(true);
    // }
  } catch (e) {
    // useAuth may not be available in all contexts (e.g., outside React tree)
  }
  // Fallback to auth.currentUser
  const user = auth.currentUser;
  if (!user || typeof user.getIdToken !== "function") {
    throw new Error("User is not logged in or getIdToken is not a function");
  }
